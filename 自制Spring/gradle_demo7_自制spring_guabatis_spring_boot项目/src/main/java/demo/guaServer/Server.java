...
public class Server {
            ...
            while (true) {
                ...
                try (Socket socket = serverSocket.accept()) {
                    ...
                    if (request.length() > 0) {
                        ...
                        Utility.log("请求:\n%s", request);
                        // 解析 request 得到 path
                        GuaRequest r = new GuaRequest(request);
    ...
    private  byte[] responseForPath(GuaRequest guaRequest) {
        GuaResponse guaResponse = new GuaResponse();
        this.servlet.doDispatch(guaRequest, guaResponse);
        return guaResponse.raw();
    }
}
